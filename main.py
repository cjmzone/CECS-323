...
def add_department(session: Session):
    ...
    while not (unique_name and unique_abbreviation and unique_chair and unique_office and unique_description):
        ...
        # Check for unique department abbreviation
        ...
        # Check for unique building and office combination
        ...
        if not unique_office:
            print("We already have a department with that building and office combination. Try again.")
            continue

        # Check for unique department description
...
def select_department(sess: Session) -> Department:
    ...
    while not found:
        ...
        if len(departments) == 1:
            ...
            print("Multiple departments found. Please be more specific.")
        else:
...
def delete_department(session: Session):
    ...
    if department:
        ...
        print(f"Deleted department: {department}")
    else:
        print("No department found to delete.")


def list_departments(session: Session):
    departments = list(session.query(Department).order_by(Department.name))
